...
def activate_subscription(db_conn, vault_id: str, plan_id: str):
    ...
    # Different plan — queue it; the existing expiry job (already scheduled
    # when the current plan was activated) will promote it automatically.
    db_conn.execute(
...
def restore_all_expiry_jobs(db_getter, bot):
    ...
    an active or future expiry. Needed because in-memory scheduled jobs are
    lost on restart — this rebuilds them from the DB (still zero polling).
    """
    with db_getter() as db:
        rows = db.execute(
