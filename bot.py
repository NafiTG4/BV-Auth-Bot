...
def _get_plan_group_for_vault(vault_id: str) -> str:
    """Return plan group (basic/plus/pro) for a vault based on active subscription."""
    try:
        from pricing import get_active_subscription
        with get_db() as c:
            sub = get_active_subscription(c, vault_id)
        if sub:
            return sub["plan_id"].split("_")[0]
    except Exception:
        pass
    return "basic"

def _get_plan_vault_max(plan_group: str):
    val = _load_setting(f"{plan_group}_vault_max", "5")
    try:
        return int(val) if val else 5
    except Exception:
        return 5

def _get_plan_per_min(plan_group: str):
    val = _load_setting(f"{plan_group}_per_min", "5")
    try:
        return int(val) if val else 5
    except Exception:
        return 5
...
def get_effective_vault_max(vault_id: str) -> int:
    """Return the effective max TOTP per vault: custom > plan > global."""
    custom, _ = get_vault_custom_limits(vault_id)
    if custom is not None:
        return custom
    plan_max = _get_plan_vault_max(_get_plan_group_for_vault(vault_id))
    if plan_max is not None:
        return plan_max
    return MAX_TOTP_PER_VAULT

def get_effective_per_min_limit(vault_id: str) -> int:
    """Return the effective per-minute TOTP limit: custom > plan > global."""
    _, custom = get_vault_custom_limits(vault_id)
    if custom is not None:
        return custom
    plan_min = _get_plan_per_min(_get_plan_group_for_vault(vault_id))
    if plan_min is not None:
        return plan_min
    return MAX_TOTP_PER_MINUTE
...
def _get_plan_export_limit(plan_group: str) -> int:
    """Return the configured daily export limit for a plan group (basic/plus/pro)."""
    val = _load_setting(f"{plan_group}_export_limit", "2")
    try:
        return int(val) if val else 2
    except Exception:
        return 2

def _get_plan_import_limit(plan_group: str) -> int:
    """Return the configured daily import limit for a plan group (basic/plus/pro)."""
    val = _load_setting(f"{plan_group}_import_limit", "3")
    try:
        return int(val) if val else 3
    except Exception:
        return 3

def is_plan_export_enabled(plan_group: str) -> bool:
    """Return True if vault export is enabled for this specific plan group only."""
    return _load_setting(f"{plan_group}_export_enabled", "1") != "0"

def is_plan_import_enabled(plan_group: str) -> bool:
    """Return True if vault import is enabled for this specific plan group only."""
    return _load_setting(f"{plan_group}_import_enabled", "1") != "0"

def get_effective_export_limit(vault_id: str) -> int:
    """Return the effective daily export limit for this vault (specific override > the vault's own plan setting)."""
    specific, _ = get_vault_ei_limits(vault_id)
    if specific is not None:
        return specific
    return _get_plan_export_limit(_get_plan_group_for_vault(vault_id))

def get_effective_import_limit(vault_id: str) -> int:
    """Return the effective daily import limit for this vault (specific override > the vault's own plan setting)."""
    _, specific = get_vault_ei_limits(vault_id)
    if specific is not None:
        return specific
    return _get_plan_import_limit(_get_plan_group_for_vault(vault_id))
...
def check_export_allowed(vault_id: str) -> tuple:
    """Returns (allowed: bool, reason: str or None). Enforced using the vault's own plan group only."""
    group = _get_plan_group_for_vault(vault_id)
    if not is_plan_export_enabled(group):
...
def check_import_allowed(vault_id: str) -> tuple:
    """Returns (allowed: bool, reason: str or None). Enforced using the vault's own plan group only."""
    group = _get_plan_group_for_vault(vault_id)
    if not is_plan_import_enabled(group):
        return False, "disabled"
...
def validate_secret(s: str):
    """
    Strict TOTP secret key validation:
    - Must be ASCII-only (no non-English / non-Latin characters)
    - After stripping common separators, must be 16–64 characters
    - Must consist ONLY of valid base32 characters: A-Z and 2-7 (case-insensitive)
    - Must be decodable as base32
    """
    # Reject immediately if any non-ASCII character is present
    try:
        s.encode("ascii")
    except (UnicodeEncodeError, UnicodeDecodeError):
        return False, ""

    c = clean_secret(s)   # strip spaces, dashes, dots, commas, underscores and uppercase

    # Length check: 16–64 characters
    if len(c) < 16 or len(c) > 64:
        return False, ""

    # Strict character check: ONLY base32 alphabet A-Z and 2-7, no substitutions
    if re.search(r"[^A-Z2-7]", c):
        return False, ""
...
async def show_profile(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        from pricing import get_active_subscription, PLANS
        with get_db() as c:
            sub = get_active_subscription(c, vault)
        if sub:
            plan_group = sub["plan_id"].split("_")[0]
            plan_name  = plan_group.capitalize()
            if sub["expires_at"]:
                expiry_text = fmt_time(sub["expires_at"], tz)
            else:
                expiry_text = "Lifetime"
    except Exception:
        pass
    text = (
        ...
        f"*TOTP Accounts:* {cnt}\n\n"
        f"*Plan :* {em(plan_name)}\n\n"
        f"*Plan Expiry Date :* {em(expiry_text)}\n\n"
        f"*Timezone:* {em(tz)}\n\n"
...
async def add_totp_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not TOTP_ADD_ENABLED:
        ...
        return TOTP_MENU
    # Plan-level TOTP add toggle check
    vault = get_session(update.effective_user.id)
    if vault:
        plan_group = _get_plan_group_for_vault(vault)
        if _load_setting(f"{plan_group}_totp_add_enabled", "1") == "0":
            await q.edit_message_text(
                "⚠️ *Adding new TOTP accounts is disabled for your current plan\\.*",
                parse_mode="MarkdownV2",
                reply_markup=kb_main(),
            )
            return TOTP_MENU
...
async def adm_premium_wallet_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    q = update.callback_query; await q.answer()
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("Get Single Private Key", callback_data="adm_wallet_get_privkey")],
        [InlineKeyboardButton("Export All Address",     callback_data="adm_wallet_export_all")],
        [InlineKeyboardButton("Check Address Balance",  callback_data="adm_wallet_check_balance")],
        [InlineKeyboardButton("⬅️ Back",                callback_data="adm_premium")],
    ])
    await q.edit_message_text("🔐 Wallet Private Info", reply_markup=kb)

async def adm_wallet_get_privkey_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Ask admin for a wallet address to retrieve its private key."""
    q = update.callback_query; await q.answer()
    _admin_import_pending[update.effective_chat.id] = {"step": "adm_wallet_privkey_wait"}
    await q.edit_message_text(
        "Send the wallet address you want to get the private key for.\n"
        "Only addresses generated by this bot are supported."
    )
...
async def adm_wallet_check_balance_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    q = update.callback_query; await q.answer()
    _admin_import_pending[update.effective_chat.id] = {"step": "adm_wallet_balance_wait"}
    await q.edit_message_text(
        "Send the wallet address you want to check the balance for."
    )
...
async def adm_premium_plan_view_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if group == "basic":
        ...
        oab_on     = _load_setting("basic_oab_enabled", "1") != "0"
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("Set Basic Message",         callback_data="adm_set_basic_msg")],
            [InlineKeyboardButton("Basic Full User Export",    callback_data="adm_basic_full_export")],
            ...
            [InlineKeyboardButton("TOTP Export & Import",      callback_data="adm_plan_ei_menu:basic")],
            [InlineKeyboardButton(f"Offline Auto Backup {'✅' if oab_on else '❌'}", callback_data="adm_oab_toggle:basic")],
        ...
        premium_on = _load_setting("plus_premium_enabled", "1") != "0"
        oab_on     = _load_setting("plus_oab_enabled", "1") != "0"
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton(f"Plus Premium {'ON' if premium_on else 'OFF'}", callback_data="adm_plus_premium_toggle")],
            ...
            [InlineKeyboardButton("TOTP Export & Import",      callback_data="adm_plan_ei_menu:plus")],
            [InlineKeyboardButton(f"Offline Auto Backup {'✅' if oab_on else '❌'}", callback_data="adm_oab_toggle:plus")],
            [InlineKeyboardButton("Plus Full User Export",     callback_data="adm_plus_full_export")],
        ...
        premium_on = _load_setting("pro_premium_enabled", "1") != "0"
        oab_on     = _load_setting("pro_oab_enabled", "1") != "0"
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton(f"Pro Premium {'ON' if premium_on else 'OFF'}", callback_data="adm_pro_premium_toggle")],
            ...
            [InlineKeyboardButton("TOTP Export & Import",      callback_data="adm_plan_ei_menu:pro")],
            [InlineKeyboardButton(f"Offline Auto Backup {'✅' if oab_on else '❌'}", callback_data="adm_oab_toggle:pro")],
            [InlineKeyboardButton("Pro Full User Export",      callback_data="adm_pro_full_export")],
...
async def adm_plus_premium_toggle_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer()
    current = _load_setting("plus_premium_enabled", "1") != "0"
    _save_setting("plus_premium_enabled", "0" if current else "1")
    new_state = not current
    await q.answer(f"Plus Premium is now {'ON' if new_state else 'OFF'}.", show_alert=True)
    q.data = "adm_premium_plan_view:plus"
    await adm_premium_plan_view_cb(update, ctx)

async def adm_pro_premium_toggle_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer()
    current = _load_setting("pro_premium_enabled", "1") != "0"
    _save_setting("pro_premium_enabled", "0" if current else "1")
    new_state = not current
    await q.answer(f"Pro Premium is now {'ON' if new_state else 'OFF'}.", show_alert=True)
    q.data = "adm_premium_plan_view:pro"
    await adm_premium_plan_view_cb(update, ctx)
    q = update.callback_query; await q.answer()
...
async def adm_oab_toggle_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Toggle offline auto backup on/off for a plan group and disable all users if turned off."""
    q     = update.callback_query; await q.answer()
    group = q.data.split(":", 1)[1]
    key   = f"{group}_oab_enabled"
    current = _load_setting(key, "1") != "0"
    new_state = not current
    _save_setting(key, "1" if new_state else "0")
    if not new_state:
        # Disable offline auto backup for all users of this plan group
        from pricing import PLANS, get_active_subscription
        plan_ids = [pid for pid, p in PLANS.items() if p.get("plan_group") == group]
        if group == "basic":
            # Basic users = users with no active paid subscription
            with get_db() as c:
                all_tids = [r["telegram_id"] for r in c.execute("SELECT telegram_id FROM users").fetchall()]
                paid_tids = set(
                    r["telegram_id"] for r in
                    c.execute(
                        "SELECT u.telegram_id FROM subscriptions s "
                        "JOIN users u ON u.vault_id = s.vault_id "
                        "WHERE s.is_active=1"
                    ).fetchall()
                )
                basic_tids = [t for t in all_tids if t not in paid_tids]
                for tid in basic_tids:
                    c.execute(
                        "UPDATE auto_backup_settings SET enabled=0 WHERE telegram_id=?", (tid,)
                    )
                c.commit()
        else:
            with get_db() as c:
                rows = c.execute(
                    f"""SELECT u.telegram_id FROM subscriptions s
                        JOIN users u ON u.vault_id = s.vault_id
                        WHERE s.plan_id IN ({','.join('?'*len(plan_ids))}) AND s.is_active=1""",
                    plan_ids,
                ).fetchall()
                for r in rows:
                    c.execute(
                        "UPDATE auto_backup_settings SET enabled=0 WHERE telegram_id=?",
                        (r["telegram_id"],)
                    )
                c.commit()
    await q.answer(
        f"Offline Auto Backup for {group.capitalize()} is now {'ON' if new_state else 'OFF'}.",
        show_alert=True,
    )
...
async def adm_add_plus_user_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer()
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("30 Day",   callback_data="adm_add_user_plan:plus_30")],
        [InlineKeyboardButton("1 Year",   callback_data="adm_add_user_plan:plus_year")],
        [InlineKeyboardButton("⬅️ Back",  callback_data="adm_premium_plan_view:plus")],
    ])
...
async def adm_plus_totp_control_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer()
    totp_on  = _load_setting("plus_totp_add_enabled", "1") != "0"
    vmax     = _load_setting("plus_vault_max", "5")
    vmin     = _load_setting("plus_per_min", "5")
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton(f"TOTP Add {'✅' if totp_on else '❌'}",        callback_data="adm_plan_totp_add_toggle:plus")],
        [InlineKeyboardButton(f"Vault TOTP Max limit ({vmax})",              callback_data="adm_plan_vault_max:plus")],
        [InlineKeyboardButton(f"Vault TOTP add limit per min ({vmin})",      callback_data="adm_plan_per_min:plus")],
        [InlineKeyboardButton("Specific User Vault Max limit",               callback_data="adm_plan_user_vault_max:plus")],
        [InlineKeyboardButton("Specific User TOTP add limit per min",        callback_data="adm_plan_user_per_min:plus")],
        [InlineKeyboardButton("⬅️ Back",                                     callback_data="adm_premium_plan_view:plus")],
    ])
...
async def adm_add_pro_user_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer()
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("30 Day",   callback_data="adm_add_user_plan:pro_30")],
        [InlineKeyboardButton("1 Year",   callback_data="adm_add_user_plan:pro_year")],
        [InlineKeyboardButton("⬅️ Back",  callback_data="adm_premium_plan_view:pro")],
    ])
    await q.edit_message_text("Add Pro User", reply_markup=kb)

async def adm_add_user_plan_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Ask admin for a vault/telegram ID to grant a specific plan duration."""
    q       = update.callback_query; await q.answer()
    plan_id = q.data.split(":", 1)[1]
    _admin_import_pending[update.effective_chat.id] = {"step": "adm_add_user_plan_id_wait", "plan_id": plan_id}
    await q.edit_message_text(
        "Send the Telegram User ID or Vault ID of the user you want to grant this plan to."
    )

async def adm_pro_totp_control_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer()
    totp_on  = _load_setting("pro_totp_add_enabled", "1") != "0"
    vmax     = _load_setting("pro_vault_max", "5")
    vmin     = _load_setting("pro_per_min", "5")
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton(f"TOTP Add {'✅' if totp_on else '❌'}",        callback_data="adm_plan_totp_add_toggle:pro")],
        [InlineKeyboardButton(f"Vault TOTP Max limit ({vmax})",              callback_data="adm_plan_vault_max:pro")],
        [InlineKeyboardButton(f"Vault TOTP add limit per min ({vmin})",      callback_data="adm_plan_per_min:pro")],
        [InlineKeyboardButton("Specific User Vault Max limit",               callback_data="adm_plan_user_vault_max:pro")],
        [InlineKeyboardButton("Specific User TOTP add limit per min",        callback_data="adm_plan_user_per_min:pro")],
        [InlineKeyboardButton("⬅️ Back",                                     callback_data="adm_premium_plan_view:pro")],
    ])
...
async def adm_premium_api_txcheck_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer()
    with get_db() as c:
        alchemy_row = c.execute("SELECT value FROM bot_settings WHERE key='alchemy_api_key'").fetchone()
        helius_row  = c.execute("SELECT value FROM bot_settings WHERE key='helius_api_key'").fetchone()
    alchemy_val = alchemy_row["value"] if alchemy_row and alchemy_row["value"] else None
    helius_val  = helius_row["value"]  if helius_row  and helius_row["value"]  else None
    alchemy_display = f"{alchemy_val[:6]}...{alchemy_val[-4:]}" if alchemy_val else "Not set"
    helius_display  = f"{helius_val[:6]}...{helius_val[-4:]}"   if helius_val  else "Not set"
    kb = InlineKeyboardMarkup([
    ...
    await q.edit_message_text(
        f"Transaction Check\n\n"
        f"Alchemy API Key: {alchemy_display}\n"
        f"Solana Helius API Key: {helius_display}",
        reply_markup=kb,
    )
...
async def adm_basic_totp_control_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer()
    totp_on  = _load_setting("basic_totp_add_enabled", "1") != "0"
    vmax     = _load_setting("basic_vault_max", "5")
    vmin     = _load_setting("basic_per_min", "5")
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton(f"TOTP Add {'✅' if totp_on else '❌'}",        callback_data="adm_plan_totp_add_toggle:basic")],
        [InlineKeyboardButton(f"Vault TOTP Max limit ({vmax})",              callback_data="adm_plan_vault_max:basic")],
        [InlineKeyboardButton(f"Vault TOTP add limit per min ({vmin})",      callback_data="adm_plan_per_min:basic")],
        [InlineKeyboardButton("Specific User Vault Max limit",               callback_data="adm_plan_user_vault_max:basic")],
        [InlineKeyboardButton("Specific User TOTP add limit per min",        callback_data="adm_plan_user_per_min:basic")],
...
# ── Plan-scoped TOTP Control handlers (Basic/Plus/Pro) ─────────────────────────

async def adm_plan_totp_add_toggle_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Toggle TOTP Add on/off for a specific plan group."""
    q     = update.callback_query; await q.answer()
    group = q.data.split(":", 1)[1]
    current = _load_setting(f"{group}_totp_add_enabled", "1") != "0"
    _save_setting(f"{group}_totp_add_enabled", "0" if current else "1")
    new_state = not current
    await q.answer(
        f"TOTP Add for {group.capitalize()} users is now {'ENABLED' if new_state else 'DISABLED'}.",
        show_alert=True,
    )
    q.data = f"adm_{group}_totp_control" if group != "basic" else "adm_basic_totp_control"
    if group == "basic":
        await adm_basic_totp_control_cb(update, ctx)
    elif group == "plus":
        await adm_plus_totp_control_cb(update, ctx)
    elif group == "pro":
        await adm_pro_totp_control_cb(update, ctx)

async def adm_plan_vault_max_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Ask admin for new vault max TOTP limit for a plan group."""
    q     = update.callback_query; await q.answer()
    group = q.data.split(":", 1)[1]
    _admin_import_pending[update.effective_chat.id] = {"step": "adm_plan_vault_max_wait", "group": group}
    cur = _load_setting(f"{group}_vault_max", "5")
    await q.edit_message_text(
        f"{group.capitalize()} Vault TOTP Max Limit\n\n"
        f"Current: {cur} TOTP per vault\n\n"
        f"Send a new number to update."
    )

async def adm_plan_per_min_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Ask admin for new per-minute TOTP add limit for a plan group."""
    q     = update.callback_query; await q.answer()
    group = q.data.split(":", 1)[1]
    _admin_import_pending[update.effective_chat.id] = {"step": "adm_plan_per_min_wait", "group": group}
    cur = _load_setting(f"{group}_per_min", "5")
    await q.edit_message_text(
        f"{group.capitalize()} TOTP Add Limit Per Minute\n\n"
        f"Current: {cur} TOTP/min per vault\n\n"
        f"Send a new number to update."
    )

async def adm_plan_user_vault_max_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Ask admin for a vault/telegram ID to set a specific user's vault max — restricted to plan group."""
    q     = update.callback_query; await q.answer()
    group = q.data.split(":", 1)[1]
    _admin_import_pending[update.effective_chat.id] = {"step": "adm_plan_user_vault_max_id_wait", "group": group}
    await q.edit_message_text(
        f"Send the Vault ID or Telegram ID of the {group.capitalize()} user "
        f"whose Vault Max Limit you want to set."
    )

async def adm_plan_user_per_min_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Ask admin for a vault/telegram ID to set a specific user's per-min limit — restricted to plan group."""
    q     = update.callback_query; await q.answer()
    group = q.data.split(":", 1)[1]
    _admin_import_pending[update.effective_chat.id] = {"step": "adm_plan_user_per_min_id_wait", "group": group}
    await q.edit_message_text(
        f"Send the Vault ID or Telegram ID of the {group.capitalize()} user "
        f"whose Per-Minute Add Limit you want to set."
    )

# ── End Plan-scoped TOTP Control handlers ───────────────────────────────────────

# ── Plan-scoped TOTP Export & Import handlers (Basic/Plus/Pro) ─────────────────
# Each plan group has its own export/import limits, on/off switches, and specific
# vault overrides. Settings made here only ever apply to that plan's own users.

def _plan_ei_menu_kb(group: str) -> InlineKeyboardMarkup:
    """Build the per-plan Export & Import menu keyboard with live on/off labels."""
    exp_label = "✅ Export: ON" if is_plan_export_enabled(group) else "🚫 Export: OFF"
    imp_label = "✅ Import: ON" if is_plan_import_enabled(group) else "🚫 Import: OFF"
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("📤 Vault Export Limit",               callback_data=f"adm_plan_ei_exp_limit:{group}")],
        [InlineKeyboardButton("📥 Vault Import Limit",               callback_data=f"adm_plan_ei_imp_limit:{group}")],
        [InlineKeyboardButton("🔒 Specific Vault Export Limit",      callback_data=f"adm_plan_ei_spec_exp:{group}")],
        [InlineKeyboardButton("🔓 Specific Vault Import Limit",      callback_data=f"adm_plan_ei_spec_imp:{group}")],
        [InlineKeyboardButton("📋 Specific Vault Export Limit List", callback_data=f"adm_plan_ei_spec_exp_list:{group}")],
        [InlineKeyboardButton("📋 Specific Vault Import Limit List", callback_data=f"adm_plan_ei_spec_imp_list:{group}")],
        [InlineKeyboardButton(exp_label,                             callback_data=f"adm_plan_ei_exp_toggle:{group}")],
        [InlineKeyboardButton(imp_label,                             callback_data=f"adm_plan_ei_imp_toggle:{group}")],
        [InlineKeyboardButton("⬅️ Back",                             callback_data=f"adm_premium_plan_view:{group}")],
    ])

async def adm_plan_ei_menu_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Show the Export & Import settings menu for one plan group (basic/plus/pro)."""
    q     = update.callback_query; await q.answer()
    group = q.data.split(":", 1)[1]
    exp_lim = _get_plan_export_limit(group)
    imp_lim = _get_plan_import_limit(group)
    await q.edit_message_text(
        f"📦 {group.capitalize()} TOTP Export & Import Settings\n\n"
        f"Export limit : {exp_lim}/day\n"
        f"Import limit : {imp_lim}/day\n"
        f"Export       : {'ON' if is_plan_export_enabled(group) else 'OFF'}\n"
        f"Import       : {'ON' if is_plan_import_enabled(group) else 'OFF'}\n\n"
        f"These settings only affect {group.capitalize()} plan users.",
        reply_markup=_plan_ei_menu_kb(group),
    )

async def adm_plan_ei_exp_limit_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Ask admin for a new daily export limit for this plan group."""
    q     = update.callback_query; await q.answer()
    group = q.data.split(":", 1)[1]
    _admin_import_pending[update.effective_chat.id] = {"step": "adm_plan_ei_exp_limit_wait", "group": group}
    cur = _get_plan_export_limit(group)
    await q.edit_message_text(
        f"{group.capitalize()} Vault Export Limit\n\n"
        f"Current: {cur}/day\n\n"
        f"Send the new daily export limit for {group.capitalize()} plan users."
    )

async def adm_plan_ei_imp_limit_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Ask admin for a new daily import limit for this plan group."""
    q     = update.callback_query; await q.answer()
    group = q.data.split(":", 1)[1]
    _admin_import_pending[update.effective_chat.id] = {"step": "adm_plan_ei_imp_limit_wait", "group": group}
    cur = _get_plan_import_limit(group)
    await q.edit_message_text(
        f"{group.capitalize()} Vault Import Limit\n\n"
        f"Current: {cur}/day\n\n"
        f"Send the new daily import limit for {group.capitalize()} plan users."
    )

async def adm_plan_ei_spec_exp_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Ask admin for a Vault/Telegram ID to set a specific export limit, restricted to this plan group."""
    q     = update.callback_query; await q.answer()
    group = q.data.split(":", 1)[1]
    _admin_import_pending[update.effective_chat.id] = {"step": "adm_plan_ei_spec_exp_id_wait", "group": group}
    await q.edit_message_text(
        f"Send the Vault ID or Telegram ID of the {group.capitalize()} user "
        f"whose Export Limit you want to set."
    )

async def adm_plan_ei_spec_imp_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Ask admin for a Vault/Telegram ID to set a specific import limit, restricted to this plan group."""
    q     = update.callback_query; await q.answer()
    group = q.data.split(":", 1)[1]
    _admin_import_pending[update.effective_chat.id] = {"step": "adm_plan_ei_spec_imp_id_wait", "group": group}
    await q.edit_message_text(
        f"Send the Vault ID or Telegram ID of the {group.capitalize()} user "
        f"whose Import Limit you want to set."
    )

async def _plan_ei_spec_list(update: Update, ctx: ContextTypes.DEFAULT_TYPE, group: str, action: str):
    """Shared logic: send a TXT file listing vault-specific export/import overrides for one plan group only."""
    q   = update.callback_query
    col = "export_limit" if action == "export" else "import_limit"
    with get_db() as c:
        rows = c.execute(
            f"SELECT vault_id, {col} FROM vault_ei_limits WHERE {col} IS NOT NULL ORDER BY vault_id"
        ).fetchall()
    # Only keep vaults that currently belong to THIS plan group, so one plan's
    # admin list never shows or leaks another plan's user overrides.
    rows = [r for r in rows if _get_plan_group_for_vault(r["vault_id"]) == group]
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Back", callback_data=f"adm_plan_ei_menu:{group}")]])
    if not rows:
        await q.edit_message_text(
            f"No specific {action} limits set for {group.capitalize()} users yet.", reply_markup=kb
        )
        return
    label = "Export Limit/day" if action == "export" else "Import Limit/day"
    lines = [f"Vault ID               | {label}", "-" * 40]
    for r in rows:
        lines.append(f"{r['vault_id']:<22} | {r[col]}")
    txt = "\n".join(lines).encode()
    bio = BytesIO(txt)
    bio.name = f"{group}_specific_{action}_limits.txt"
    await q.message.reply_document(document=bio, filename=bio.name)
    await q.edit_message_text(
        f"✅ Listed {len(rows)} {group.capitalize()} vault(s) with a specific {action} limit.", reply_markup=kb
    )

async def adm_plan_ei_spec_exp_list_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    q     = update.callback_query; await q.answer()
    group = q.data.split(":", 1)[1]
    await _plan_ei_spec_list(update, ctx, group, "export")

async def adm_plan_ei_spec_imp_list_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    q     = update.callback_query; await q.answer()
    group = q.data.split(":", 1)[1]
    await _plan_ei_spec_list(update, ctx, group, "import")

async def adm_plan_ei_exp_toggle_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Toggle export on/off for this plan group only. Other plans are never affected."""
    q       = update.callback_query; await q.answer()
    group   = q.data.split(":", 1)[1]
    current = is_plan_export_enabled(group)
    _save_setting(f"{group}_export_enabled", "0" if current else "1")
    await q.answer(f"{group.capitalize()} Export is now {'OFF' if current else 'ON'}.", show_alert=True)
    q.data = f"adm_plan_ei_menu:{group}"
    await adm_plan_ei_menu_cb(update, ctx)

async def adm_plan_ei_imp_toggle_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Toggle import on/off for this plan group only. Other plans are never affected."""
    q       = update.callback_query; await q.answer()
    group   = q.data.split(":", 1)[1]
    current = is_plan_import_enabled(group)
    _save_setting(f"{group}_import_enabled", "0" if current else "1")
    await q.answer(f"{group.capitalize()} Import is now {'OFF' if current else 'ON'}.", show_alert=True)
    q.data = f"adm_plan_ei_menu:{group}"
    await adm_plan_ei_menu_cb(update, ctx)

# ── End Plan-scoped TOTP Export & Import handlers ───────────────────────────────

async def _export_plan_users_csv(update: Update, ctx: ContextTypes.DEFAULT_TYPE, group: str, label: str):
    """Shared logic: export Vault ID + TOTP count CSV for all users of a plan group."""
    q = update.callback_query
    if group == "basic":
        with get_db() as c:
            rows = c.execute(
                """SELECT u.vault_id, COUNT(t.id) AS totp_count
                   FROM users u
                   LEFT JOIN totp_accounts t ON t.vault_id = u.vault_id
                   LEFT JOIN subscriptions s ON s.vault_id = u.vault_id AND s.is_active = 1
        ...
        with get_db() as c:
            rows = c.execute(
                f"""SELECT u.vault_id, COUNT(t.id) AS totp_count
                    FROM users u
                    JOIN subscriptions s ON s.vault_id = u.vault_id AND s.is_active = 1
                    LEFT JOIN totp_accounts t ON t.vault_id = u.vault_id
    ...
    if not rows:
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Back", callback_data=f"adm_premium_plan_view:{group}")]])
        await q.edit_message_text(f"No {label} users found.", reply_markup=kb)
        return
    lines = [f"{i}: {r['vault_id']} - {r['totp_count']} TOTP" for i, r in enumerate(rows, start=1)]
    content = "\n".join(lines) + "\n"
    bio = BytesIO(content.encode("utf-8"))
    bio.name = f"{group}_users.csv"
    await q.message.reply_document(
        document=bio,
        filename=f"{group}_users.csv",
        caption=f"📄 {label} Users — {len(rows)} total",
    )
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Back", callback_data=f"adm_premium_plan_view:{group}")]])
    await q.edit_message_text(f"✅ Exported {len(rows)} {label} users.", reply_markup=kb)

async def adm_basic_full_export_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer()
    await _export_plan_users_csv(update, ctx, "basic", "Basic")

async def adm_plus_full_export_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer()
    await _export_plan_users_csv(update, ctx, "plus", "Plus")

async def adm_pro_full_export_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer()
    await _export_plan_users_csv(update, ctx, "pro", "Pro")
...
def _fmt_user_info(u) -> str:
    ...
    try:
        ...
        try:
            from pricing import get_active_subscription, get_plan
            with get_db() as _pc:
                sub = get_active_subscription(_pc, vault_id)
            if sub:
                plan = get_plan(sub["plan_id"])
                if plan:
                    dd = plan["duration_days"]
                    if dd == 0:
                        plan_display = plan["name"]
                    elif dd == 365:
                        plan_display = f"{plan['name']} 1 Year"
                    elif dd == 30:
                        plan_display = f"{plan['name']} 30 Day"
                    else:
                        plan_display = f"{plan['name']} {dd} Day(s)"
                duration_text = fmt_bd_time(sub["expires_at"]) if sub["expires_at"] else "Lifetime"
        except Exception as _pe:
            logger.error(f"_fmt_user_info plan lookup error: {_pe}")
...
async def admin_group_message_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if step == "adm_add_user_plan_id_wait":
        ...
        if not u:
            ...
            return
        vault_id = u["vault_id"]
        from pricing import get_plan, PLAN_REQUEST_EXPIRY_SECONDS
        import secrets as _secrets
        plan = get_plan(plan_id)
        if not plan:
            msg = await update.message.reply_text(f"Invalid plan: {plan_id}")
            asyncio.create_task(auto_delete_msg(msg, delay=60))
            return
        plan_label = plan["name"]
        dur_label  = "30 Days" if plan_id.endswith("_30") else "1 Year"
        uname = f"@{u['tg_username']}" if u["tg_username"] else str(u["telegram_id"])
        now        = int(time.time())
        request_id = _secrets.token_hex(12)
        expires_at = now + PLAN_REQUEST_EXPIRY_SECONDS
        with get_db() as c:
            c.execute(
                "INSERT INTO plan_requests (request_id, vault_id, plan_id, status, created_at, expires_at) "
                "VALUES (?,?,?,?,?,?)",
                (request_id, vault_id, plan_id, "pending", now, expires_at),
            )
            c.commit()
        try:
            from pricing import schedule_request_expiry_job
            schedule_request_expiry_job(get_db, request_id, expires_at)
        except Exception:
            pass
        msg = await update.message.reply_text(
            f"📨 Plan request sent to {uname} for {plan_label} ({dur_label}).\n"
            f"Vault: {vault_id}\nThey have 72 hours to accept."
        )
        asyncio.create_task(auto_delete_msg(msg, delay=120))
        try:
            req_kb = InlineKeyboardMarkup([[
                InlineKeyboardButton("✅ Accept",  callback_data=f"sub_req_accept:{request_id}"),
                InlineKeyboardButton("❌ Decline", callback_data=f"sub_req_decline:{request_id}"),
            ]])
            await ctx.bot.send_message(
                chat_id=u["telegram_id"],
                text=(
                    f"🎁 You have been offered the *{plan_label}* plan ({dur_label}) by an administrator.\n\n"
                    f"Accept within 72 hours or this offer will expire."
                ),
                parse_mode="Markdown",
                reply_markup=req_kb,
            )
    ...
    if step == "adm_plan_vault_max_wait":
        ...
        if not raw.strip().isdigit() or int(raw.strip()) < 1:
            ...
            asyncio.create_task(auto_delete_msg(msg, delay=60))
            return
        _save_setting(f"{group}_vault_max", raw.strip())
        success_msg = await ctx.bot.send_message(
            chat_id=chat_id,
            text=f"✅ {group.capitalize()} Vault TOTP Max Limit set to {raw.strip()}.",
        )
        asyncio.create_task(auto_delete_msg(success_msg, delay=10))
        return

    if step == "adm_plan_per_min_wait":
        group = state.get("group", "basic")
        _admin_import_pending.pop(chat_id, None)
        asyncio.create_task(auto_delete_msg(update.message, delay=5))
        if not raw.strip().isdigit() or int(raw.strip()) < 1:
            ...
            asyncio.create_task(auto_delete_msg(msg, delay=60))
            return
        _save_setting(f"{group}_per_min", raw.strip())
        success_msg = await ctx.bot.send_message(
            chat_id=chat_id,
            text=f"✅ {group.capitalize()} TOTP Add Limit Per Minute set to {raw.strip()}.",
        )
        asyncio.create_task(auto_delete_msg(success_msg, delay=10))
        return

    if step == "adm_plan_user_vault_max_id_wait":
        group = state.get("group", "basic")
        asyncio.create_task(auto_delete_msg(update.message, delay=5))
        u = _resolve_user(raw)
        if not u:
            ...
            return
        vault_id = u["vault_id"]
        user_group = _get_plan_group_for_vault(vault_id)
        if user_group != group:
            _admin_import_pending.pop(chat_id, None)
            msg = await update.message.reply_text(
                f"This vault belongs to the {user_group.capitalize()} plan, not {group.capitalize()}.\n"
                f"This limit can only be set for {group.capitalize()} plan users."
            )
            asyncio.create_task(auto_delete_msg(msg, delay=90))
            return
        cur_limit = get_effective_vault_max(vault_id)
        _admin_import_pending[chat_id] = {"step": "adm_plan_user_vault_max_val_wait", "group": group, "vault_id": vault_id}
        msg = await update.message.reply_text(
            f"Current Vault Max Limit for this {group.capitalize()} user: {cur_limit} TOTP.\n"
            f"Send a new number to update."
        )
        asyncio.create_task(auto_delete_msg(msg, delay=120))
        return

    if step == "adm_plan_user_vault_max_val_wait":
        group    = state.get("group", "basic")
        vault_id = state.get("vault_id", "")
        _admin_import_pending.pop(chat_id, None)
        asyncio.create_task(auto_delete_msg(update.message, delay=5))
        if not raw.strip().isdigit() or int(raw.strip()) < 1 or not vault_id:
            msg = await update.message.reply_text("Invalid. Send a positive integer.")
            asyncio.create_task(auto_delete_msg(msg, delay=60))
            return
        set_vault_max_limit(vault_id, int(raw.strip()))
        msg = await update.message.reply_text(
            f"✅ Vault Max Limit for {vault_id} ({group.capitalize()} plan) set to {raw.strip()}."
        )
        asyncio.create_task(auto_delete_msg(msg, delay=60))
        return

    if step == "adm_plan_user_per_min_id_wait":
        group = state.get("group", "basic")
        asyncio.create_task(auto_delete_msg(update.message, delay=5))
        u = _resolve_user(raw)
        if not u:
        ...
        _admin_import_pending[chat_id] = {"step": "adm_plan_user_per_min_val_wait", "group": group, "vault_id": vault_id}
        msg = await update.message.reply_text(
            f"Current Per-Minute Add Limit for this {group.capitalize()} user: {cur_limit} TOTP/min.\n"
            f"Send a new number to update."
        )
        asyncio.create_task(auto_delete_msg(msg, delay=120))
        return

    if step == "adm_plan_user_per_min_val_wait":
        group    = state.get("group", "basic")
        vault_id = state.get("vault_id", "")
        _admin_import_pending.pop(chat_id, None)
        asyncio.create_task(auto_delete_msg(update.message, delay=5))
        if not raw.strip().isdigit() or int(raw.strip()) < 1 or not vault_id:
            ...
            asyncio.create_task(auto_delete_msg(msg, delay=60))
            return
        set_vault_per_min_limit(vault_id, int(raw.strip()))
        msg = await update.message.reply_text(
            f"✅ Per-Minute Add Limit for {vault_id} ({group.capitalize()} plan) set to {raw.strip()}."
        )
        asyncio.create_task(auto_delete_msg(msg, delay=60))
        return
    ...
    if step == "adm_plan_ei_exp_limit_wait":
        group = state.get("group", "basic")
        _admin_import_pending.pop(chat_id, None)
        asyncio.create_task(auto_delete_msg(update.message, delay=5))
        if not raw.isdigit() or int(raw) < 1:
            ...
            asyncio.create_task(auto_delete_msg(msg, delay=60))
            return
        _save_setting(f"{group}_export_limit", raw)
        msg = await update.message.reply_text(f"✅ {group.capitalize()} daily export limit set to {raw}.")
        asyncio.create_task(auto_delete_msg(msg, delay=60))
        return

    if step == "adm_plan_ei_imp_limit_wait":
        group = state.get("group", "basic")
        _admin_import_pending.pop(chat_id, None)
        asyncio.create_task(auto_delete_msg(update.message, delay=5))
        if not raw.isdigit() or int(raw) < 1:
            ...
            asyncio.create_task(auto_delete_msg(msg, delay=60))
            return
        _save_setting(f"{group}_import_limit", raw)
        msg = await update.message.reply_text(f"✅ {group.capitalize()} daily import limit set to {raw}.")
        asyncio.create_task(auto_delete_msg(msg, delay=60))
        return

    if step == "adm_plan_ei_spec_exp_id_wait":
        group = state.get("group", "basic")
        asyncio.create_task(auto_delete_msg(update.message, delay=5))
        u = _resolve_user(raw)
        if not u:
        ...
        cur_label  = cur_exp if cur_exp is not None else _get_plan_export_limit(group)
        _admin_import_pending[chat_id] = {"step": "adm_plan_ei_spec_exp_val_wait", "group": group, "vault_id": vault_id}
        msg = await update.message.reply_text(
            f"This {group.capitalize()} vault currently has a limit of {cur_label} exports/day.\n"
            f"Send a new number to update."
        )
        asyncio.create_task(auto_delete_msg(msg, delay=120))
        return

    if step == "adm_plan_ei_spec_exp_val_wait":
        group    = state.get("group", "basic")
        vault_id = state.get("vault_id", "")
        _admin_import_pending.pop(chat_id, None)
        asyncio.create_task(auto_delete_msg(update.message, delay=5))
        ...
        msg = await update.message.reply_text(
            f"✅ Export limit for {vault_id} ({group.capitalize()} plan) set to {raw}/day."
        )
        asyncio.create_task(auto_delete_msg(msg, delay=60))
        return

    if step == "adm_plan_ei_spec_imp_id_wait":
        group = state.get("group", "basic")
        asyncio.create_task(auto_delete_msg(update.message, delay=5))
        u = _resolve_user(raw)
        if not u:
        ...
        cur_label  = cur_imp if cur_imp is not None else _get_plan_import_limit(group)
        _admin_import_pending[chat_id] = {"step": "adm_plan_ei_spec_imp_val_wait", "group": group, "vault_id": vault_id}
        msg = await update.message.reply_text(
            f"This {group.capitalize()} vault currently has a limit of {cur_label} imports/day.\n"
            f"Send a new number to update."
        )
        asyncio.create_task(auto_delete_msg(msg, delay=120))
        return

    if step == "adm_plan_ei_spec_imp_val_wait":
        group    = state.get("group", "basic")
        vault_id = state.get("vault_id", "")
        _admin_import_pending.pop(chat_id, None)
        asyncio.create_task(auto_delete_msg(update.message, delay=5))
        ...
        msg = await update.message.reply_text(
            f"✅ Import limit for {vault_id} ({group.capitalize()} plan) set to {raw}/day."
        )
        asyncio.create_task(auto_delete_msg(msg, delay=60))
        return
    ...
    if step == "adm_wallet_balance_wait":
        _admin_import_pending.pop(chat_id, None)
        asyncio.create_task(auto_delete_msg(update.message, delay=5))
        addr = raw.strip()
        with get_db() as c:
            ua_row = c.execute(
                "SELECT vault_id FROM used_addresses WHERE address=?", (addr,)
            ).fetchone()
        if not ua_row:
            msg = await update.message.reply_text(
                f"Address not found in our database:\n{addr}"
            )
            asyncio.create_task(auto_delete_msg(msg, delay=60))
            return
        with get_db() as c:
            inv_row = c.execute(
                "SELECT chain_id, token, amount_usd, status FROM payment_invoices WHERE address=? ORDER BY created_at DESC LIMIT 1",
                (addr,)
            ).fetchone()
        if not inv_row:
            msg = await update.message.reply_text(
                f"Address is in our database but has no invoice record:\n{addr}"
            )
            asyncio.create_task(auto_delete_msg(msg, delay=60))
            return
        from pricing import get_chain
        chain = get_chain(inv_row["chain_id"])
        chain_name = chain["name"] if chain else inv_row["chain_id"]
        amount_str = f"${inv_row['amount_usd']:.2f}" if inv_row["amount_usd"] else "N/A"
        status_map = {"pending": "Pending", "paid": "Completed", "expired": "Canceled"}
        status_str = status_map.get(inv_row["status"], inv_row["status"].capitalize())
        result = (
            f"Address Balance Check\n\n"
            f"Address  : {addr}\n"
            f"Chain    : {chain_name}\n"
            f"Token    : {inv_row['token']}\n"
            f"Amount   : {amount_str}\n"
            f"Status   : {status_str}"
        )
    ...
    if step == "adm_wallet_privkey_wait":
        _admin_import_pending.pop(chat_id, None)
        asyncio.create_task(auto_delete_msg(update.message, delay=5))
        addr = raw.strip()
        # Look up address in used_addresses table
        with get_db() as c:
            row = c.execute(
                "SELECT vault_id FROM used_addresses WHERE address=?", (addr,)
            ).fetchone()
        if not row:
            msg = await update.message.reply_text(
                f"Address not found in database: {addr}\n"
                "Only addresses generated by this bot are supported."
            )
            asyncio.create_task(auto_delete_msg(msg, delay=60))
            return
        # Look up invoice to get chain and address_index
        with get_db() as c:
            inv_row = c.execute(
                "SELECT chain_id, address_index FROM payment_invoices WHERE address=? LIMIT 1",
                (addr,)
            ).fetchone()
        if not inv_row:
            msg = await update.message.reply_text(
                f"No invoice record found for address: {addr}"
            )
            asyncio.create_task(auto_delete_msg(msg, delay=60))
            return
        # Derive private key
        try:
            ...
            logger.error(f"Private key derivation error: {e}")
            msg = await update.message.reply_text(f"Failed to derive private key: {e}")
            asyncio.create_task(auto_delete_msg(msg, delay=60))
            return
        chain_name = chain["name"] if chain else inv_row["chain_id"]
        result = (
            ...
            f"Address   : `{em(addr)}`\n"
            f"Chain     : {em(chain_name)}\n"
            f"Vault     : `{em(row['vault_id'])}`\n\n"
            f"Private Key:\n||{em(key_display)}||\n\n"
            f"Keep this private key secure\\. Do not share it with anyone\\."
        )
        msg = await update.message.reply_text(result, parse_mode="MarkdownV2")
        asyncio.create_task(auto_delete_msg(msg, delay=120))
        return
...
async def oab_toggle(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Toggle offline auto-backup on/off with plan permission check."""
    q   = update.callback_query
    await q.answer()
    uid   = update.effective_user.id
    vault = get_session(uid)
    # Check if user's plan allows offline auto backup
    try:
        from pricing import get_active_subscription
        with get_db() as _pc:
            sub = get_active_subscription(_pc, vault) if vault else None
        plan_group = sub["plan_id"].split("_")[0] if sub else "basic"
        oab_allowed = _load_setting(f"{plan_group}_oab_enabled", "1") != "0"
    except Exception:
        oab_allowed = True
    with get_db() as c:
        row = c.execute(
    ...
    # If trying to turn ON but plan doesn't allow it
    if not currently_enabled and not oab_allowed:
        await q.answer(
            "Please upgrade your current plan or contact support.",
            show_alert=True,
        )
        return TOTP_MENU
    with get_db() as c:
...
def main():
    ...
    if ADMIN_GROUP_ID != 0:
        ...
        app.add_handler(CallbackQueryHandler(_admin_cbq_guard(adm_wallet_get_privkey_cb),         pattern="^adm_wallet_get_privkey$"))
        app.add_handler(CallbackQueryHandler(_admin_cbq_guard(adm_wallet_export_all_cb),          pattern="^adm_wallet_export_all$"))
        app.add_handler(CallbackQueryHandler(_admin_cbq_guard(adm_wallet_check_balance_cb),       pattern="^adm_wallet_check_balance$"))
        app.add_handler(CallbackQueryHandler(_admin_cbq_guard(adm_plus_premium_toggle_cb),        pattern="^adm_plus_premium_toggle$"))
        app.add_handler(CallbackQueryHandler(_admin_cbq_guard(adm_pro_premium_toggle_cb),         pattern="^adm_pro_premium_toggle$"))
        ...
        app.add_handler(CallbackQueryHandler(_admin_cbq_guard(adm_oab_toggle_cb),                 pattern="^adm_oab_toggle:"))
        app.add_handler(CallbackQueryHandler(_admin_cbq_guard(adm_plan_totp_add_toggle_cb),       pattern="^adm_plan_totp_add_toggle:"))
        app.add_handler(CallbackQueryHandler(_admin_cbq_guard(adm_plan_vault_max_cb),             pattern="^adm_plan_vault_max:"))
        app.add_handler(CallbackQueryHandler(_admin_cbq_guard(adm_plan_per_min_cb),               pattern="^adm_plan_per_min:"))
        app.add_handler(CallbackQueryHandler(_admin_cbq_guard(adm_plan_user_vault_max_cb),        pattern="^adm_plan_user_vault_max:"))
        app.add_handler(CallbackQueryHandler(_admin_cbq_guard(adm_plan_user_per_min_cb),          pattern="^adm_plan_user_per_min:"))
        app.add_handler(CallbackQueryHandler(_admin_cbq_guard(adm_plan_ei_menu_cb),                pattern="^adm_plan_ei_menu:"))
        app.add_handler(CallbackQueryHandler(_admin_cbq_guard(adm_plan_ei_exp_limit_cb),           pattern="^adm_plan_ei_exp_limit:"))
        app.add_handler(CallbackQueryHandler(_admin_cbq_guard(adm_plan_ei_imp_limit_cb),           pattern="^adm_plan_ei_imp_limit:"))
        app.add_handler(CallbackQueryHandler(_admin_cbq_guard(adm_plan_ei_spec_exp_cb),            pattern="^adm_plan_ei_spec_exp:"))
        app.add_handler(CallbackQueryHandler(_admin_cbq_guard(adm_plan_ei_spec_imp_cb),            pattern="^adm_plan_ei_spec_imp:"))
        app.add_handler(CallbackQueryHandler(_admin_cbq_guard(adm_plan_ei_spec_exp_list_cb),       pattern="^adm_plan_ei_spec_exp_list:"))
        app.add_handler(CallbackQueryHandler(_admin_cbq_guard(adm_plan_ei_spec_imp_list_cb),       pattern="^adm_plan_ei_spec_imp_list:"))
        app.add_handler(CallbackQueryHandler(_admin_cbq_guard(adm_plan_ei_exp_toggle_cb),          pattern="^adm_plan_ei_exp_toggle:"))
        app.add_handler(CallbackQueryHandler(_admin_cbq_guard(adm_plan_ei_imp_toggle_cb),          pattern="^adm_plan_ei_imp_toggle:"))
        app.add_handler(CallbackQueryHandler(_admin_cbq_guard(adm_basic_full_export_cb),          pattern="^adm_basic_full_export$"))
        app.add_handler(CallbackQueryHandler(_admin_cbq_guard(adm_plus_full_export_cb),           pattern="^adm_plus_full_export$"))
        app.add_handler(CallbackQueryHandler(_admin_cbq_guard(adm_pro_full_export_cb),            pattern="^adm_pro_full_export$"))
        ...
        app.add_handler(CallbackQueryHandler(_admin_cbq_guard(adm_add_user_plan_cb),              pattern="^adm_add_user_plan:"))
