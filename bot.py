...
async def admin_group_message_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if step == "adm_add_user_plan_id_wait":
        ...
        if not u:
            ...
            return
        vault_id = u["vault_id"]
        from pricing import get_plan, get_active_subscription, activate_subscription
        ...
        if not plan:
            ...
            return
        with get_db() as c:
            existing = get_active_subscription(c, vault_id)
            activate_subscription(c, vault_id, plan_id)
        plan_label = plan["name"]
        dur_label  = "30 Days" if plan_id.endswith("_30") else "1 Year"
        uname = f"@{u['tg_username']}" if u["tg_username"] else str(u["telegram_id"])
        if existing and existing["plan_id"] != plan_id:
            note = (f"Note: {uname} already has an active {existing['plan_id']} plan. "
                    f"{plan_label} ({dur_label}) has been queued and will start automatically once it expires.")
        elif existing and existing["plan_id"] == plan_id:
            note = f"Existing {plan_label} plan extended by {dur_label}."
        else:
            note = f"{plan_label} ({dur_label}) activated immediately."
        msg = await update.message.reply_text(
            f"✅ {note}\nVault: {vault_id} ({uname})"
        )
        asyncio.create_task(auto_delete_msg(msg, delay=120))
        # Notify the user
        try:
            await ctx.bot.send_message(
                chat_id=u["telegram_id"],
                text=f"🎉 You have been granted the *{plan_label}* plan ({dur_label}) by an administrator.",
                parse_mode="Markdown",
            )
        except Exception:
            pass
