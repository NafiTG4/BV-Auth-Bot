...
PLAN_REQUEST_EXPIRY_SECONDS: int = 72 * 60 * 60   # 72 hours — admin-granted plan acceptance window
...
_DB_GETTER = None   # set by register_subscription_handlers
_BOT       = None   # set by register_subscription_handlers

_PLAN_TIER_ORDER = {"basic": 0, "plus": 1, "pro": 2}

def _plan_group_of(plan_id: str) -> str:
    p = get_plan(plan_id)
    return p.get("plan_group", "basic") if p else "basic"

def _plan_tier(plan_id: str) -> int:
    return _PLAN_TIER_ORDER.get(_plan_group_of(plan_id), 0)
...
def activate_subscription(db_conn, vault_id: str, plan_id: str):
    ...
    - SAME plan_id: extend its expiry.
    - DIFFERENT plan, HIGHER tier (e.g. Plus -> Pro): switch immediately,
      discarding the remainder of the current plan.
    - DIFFERENT plan, SAME or LOWER tier (e.g. Pro -> Plus, or same tier
      different duration): queue as pending_plan_id; activates once current expires.
    An exact-time job is scheduled for the new expiry — no polling involved.
    """
    ...
    if not existing:
        ...
        db_conn.commit()
        if expires and _DB_GETTER and _BOT:
            schedule_plan_expiry_job(_DB_GETTER, _BOT, vault_id, expires)
        return
    ...
    new_tier      = _plan_tier(plan_id)
    existing_tier = _plan_tier(existing["plan_id"])

    if new_tier > existing_tier:
        # Upgrade to a higher-tier plan — switch immediately, discard remainder
        expires = None if plan["duration_days"] == 0 else now + plan["duration_days"] * 86400
        db_conn.execute(
            """UPDATE subscriptions
               SET plan_id=?, activated_at=?, expires_at=?, is_active=1, pending_plan_id=NULL
               WHERE vault_id=?""",
            (plan_id, now, expires, vault_id),
        )
        db_conn.commit()
        if expires and _DB_GETTER and _BOT:
            schedule_plan_expiry_job(_DB_GETTER, _BOT, vault_id, expires)
    ...
    # Same tier different plan, or downgrade — queue it; the existing expiry
    # job (already scheduled when the current plan was activated) will
    # promote it automatically once the current plan ends.
    db_conn.execute(
...
def activate_pending_plan_if_expired(db_conn, vault_id: str):
    ...
    If so, activate the queued plan starting now. Called exactly once at the
    scheduled expiry moment (see schedule_plan_expiry_job) — never polled.
    """
    now = int(time.time())
    row = db_conn.execute(
...
# ─────────────────────────────────────────────────────────────────────────────
# EXACT-TIME EXPIRY SCHEDULER (no polling — one job fires exactly at expiry)
# ─────────────────────────────────────────────────────────────────────────────
_scheduler = None   # set by register_subscription_handlers

def _get_scheduler():
    """Lazily create a single shared AsyncIOScheduler instance."""
    global _scheduler
...
async def _on_plan_expiry(db_getter, bot, vault_id: str):
    """
    Fired exactly once at a vault's plan expiry timestamp.
    Promotes the pending plan if one is queued, deactivates otherwise,
    and notifies the user. Pure DB work — no external API calls.
    """
    try:
        with db_getter() as db:
            before = db.execute(
                "SELECT pending_plan_id FROM subscriptions WHERE vault_id=?", (vault_id,)
            ).fetchone()
            had_pending = bool(before and before["pending_plan_id"])
            activate_pending_plan_if_expired(db, vault_id)
        if not had_pending:
            return  # plan simply ended, nothing to promote/notify
        with db_getter() as db:
            sub_row  = db.execute(
                "SELECT plan_id, expires_at FROM subscriptions WHERE vault_id=?", (vault_id,)
            ).fetchone()
            user_row = db.execute(
                "SELECT telegram_id FROM users WHERE vault_id=?", (vault_id,)
            ).fetchone()
        if sub_row and user_row:
            ...
            # Chain-schedule the next expiry in case this newly-promoted plan
            # itself has a duration (so it deactivates/promotes correctly later)
            if sub_row["expires_at"]:
                schedule_plan_expiry_job(db_getter, bot, vault_id, sub_row["expires_at"])
    except Exception as e:
        logger.error(f"Plan expiry job error for {vault_id}: {e}")

def schedule_plan_expiry_job(db_getter, bot, vault_id: str, expires_at: int):
    """
    Schedule a one-shot job to fire exactly at `expires_at` (unix timestamp).
    Replaces any previously scheduled job for this vault. This means zero
    polling/API cost — the job runs once, precisely when the plan ends.
    """
    import datetime as _dt
    sched   = _get_scheduler()
    job_id  = f"plan_expiry_{vault_id}"
    run_at  = _dt.datetime.fromtimestamp(expires_at, tz=_dt.timezone.utc)
    try:
        sched.remove_job(job_id)
    except Exception:
        pass
    # Don't schedule jobs in the past (would fire immediately on next loop tick)
    now = _dt.datetime.now(tz=_dt.timezone.utc)
    if run_at <= now:
        run_at = now + _dt.timedelta(seconds=2)
    sched.add_job(
        _on_plan_expiry,
        "date",
        run_date=run_at,
        args=[db_getter, bot, vault_id],
        id=job_id,
        replace_existing=True,
        misfire_grace_time=3600,  # tolerate up to 1h delay if bot was offline
    )

def restore_all_expiry_jobs(db_getter, bot):
    """
    Call once at startup to re-schedule expiry jobs for every vault that has
    an active or future expiry, and every still-pending plan request.
    Needed because in-memory scheduled jobs are lost on restart — this
    rebuilds them from the DB (still zero polling).
    """
    with db_getter() as db:
        rows = db.execute(
            "SELECT vault_id, expires_at FROM subscriptions WHERE expires_at IS NOT NULL"
        ).fetchall()
    for row in rows:
        schedule_plan_expiry_job(db_getter, bot, row["vault_id"], row["expires_at"])

    with db_getter() as db:
        req_rows = db.execute(
            "SELECT request_id, expires_at FROM plan_requests WHERE status='pending'"
        ).fetchall()
    for row in req_rows:
        schedule_request_expiry_job(db_getter, row["request_id"], row["expires_at"])
...
# ═════════════════════════════════════════════════════════════════════════════
# PLAN REQUEST ACCEPT/DECLINE  (admin-granted plans require user confirmation)
# ═════════════════════════════════════════════════════════════════════════════
async def cb_sub_req_accept(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """User accepts an admin-granted plan request."""
    from bot import get_db
    q          = update.callback_query; await q.answer()
    request_id = q.data.split(":", 1)[1]
    now        = int(time.time())
    with get_db() as db:
        row = db.execute(
            "SELECT * FROM plan_requests WHERE request_id=?", (request_id,)
        ).fetchone()
    if not row:
        await q.edit_message_text("This request no longer exists.")
        return
    req = dict(row)
    if req["status"] != "pending":
        await q.edit_message_text(f"This request has already been {req['status']}.")
        return
    if req["expires_at"] <= now:
        with get_db() as db:
            db.execute(
                "UPDATE plan_requests SET status='expired' WHERE request_id=?", (request_id,)
            )
            db.commit()
        await q.edit_message_text("⌛ This plan offer has expired.")
        return
    plan = get_plan(req["plan_id"])
    with get_db() as db:
        activate_subscription(db, req["vault_id"], req["plan_id"])
        db.execute(
            "UPDATE plan_requests SET status='accepted', responded_at=? WHERE request_id=?",
            (now, request_id),
        )
        db.commit()
    dur_label = "30 Days" if req["plan_id"].endswith("_30") else "1 Year"
    await q.edit_message_text(
        f"✅ You have accepted the *{plan['name']}* plan ({dur_label}).\nIt is now active!",
        parse_mode="Markdown",
    )

async def cb_sub_req_decline(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """User declines an admin-granted plan request."""
    from bot import get_db
    q          = update.callback_query; await q.answer()
    request_id = q.data.split(":", 1)[1]
    with get_db() as db:
    ...
    with get_db() as db:
        db.execute(
            "UPDATE plan_requests SET status='declined', responded_at=? WHERE request_id=?",
            (int(time.time()), request_id),
        )
        db.commit()
    await q.edit_message_text("❌ You have declined this plan offer.")

async def _on_request_expiry(db_getter, request_id: str):
    """Fired exactly at a plan request's 72h expiry — marks it expired if still pending."""
    try:
        with db_getter() as db:
            db.execute(
                "UPDATE plan_requests SET status='expired' WHERE request_id=? AND status='pending'",
                (request_id,),
            )
            db.commit()
    except Exception as e:
        logger.error(f"Plan request expiry job error: {e}")

def schedule_request_expiry_job(db_getter, request_id: str, expires_at: int):
    """Schedule a one-shot job to auto-expire a plan request at its deadline."""
    import datetime as _dt
    sched  = _get_scheduler()
    job_id = f"plan_request_expiry_{request_id}"
    run_at = _dt.datetime.fromtimestamp(expires_at, tz=_dt.timezone.utc)
    now    = _dt.datetime.now(tz=_dt.timezone.utc)
    if run_at <= now:
        run_at = now + _dt.timedelta(seconds=2)
...
def register_subscription_handlers(app, db_getter):
    ...
    app.add_handler(CallbackQueryHandler(cb_sub_req_accept,  pattern=r"^sub_req_accept:"))
    app.add_handler(CallbackQueryHandler(cb_sub_req_decline, pattern=r"^sub_req_decline:"))
    ...
    async def _start_poller(application):
        ...
        # Rebuild exact-time expiry jobs from DB (in-memory schedule is lost on restart)
        restore_all_expiry_jobs(db_getter, application.bot)
